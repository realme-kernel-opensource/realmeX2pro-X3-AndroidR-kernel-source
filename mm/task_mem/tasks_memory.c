...
static int memory_monitor_show(struct seq_file *m, void *p)
{
	...
	seq_printf(m, "%-24s\t%-24s\t%-24s\t%-24s\t%-24s\t%-24s\t%-24s\t%-24s\t%-24s\n",
			"task_name", "pid", "uid", "oom_score_adj", "rss", "swapents_ori", "ion", "GL+dev", "rssfile");
	...
	for (i = 0; i < record_tasks; i++) {
		...
		seq_printf(m, "%-24s\t%-24d\t%-24u\t%-24hd\t%-24lu\t%-24lu\t%-24lu\t%-24lu\t%-24lu\n",
				(char *)(pmem[i].comm), pmem[i].pid, pmem[i].uid,
				pmem[i].oom_score_adj, pmem[i].rss, pmem[i].swapents_ori,
				pmem[i].ions, pmem[i].gl_dev, pmem[i].rssfile);
	}
	end = sched_clock();
